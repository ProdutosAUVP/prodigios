...
- Cor: preto (`ink`), grafite (`graphite`) e papel. **Sem verde escuro** (`forest`/`mint`/`primary` não se usam). Lime **só** em botão CTA, check de sucesso, ponto do loader e destaques pontuais (palavras-chave da Intro, números da lista "O que você vai aprender", ícone do card de trilha no hover). Ênfase de texto é opacidade.
