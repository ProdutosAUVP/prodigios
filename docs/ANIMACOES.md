...
| Intro | dobra grafite; palavras `opacity 0.18 → 1` com `scrub: 0.4`; um segundo `ScrollTrigger` (`toggleClass: is-lit`, em `bottom 65%`, o mesmo ponto em que o scrub termina) acende as palavras-chave em lime **só depois da frase inteira revelada**, ainda com a frase no meio da tela (transição CSS de 350 ms) | `Intro.tsx` |
| Process | lista vertical; um `ScrollTrigger` por fase (`start: top 55%`, `end: bottom 55%`) liga `is-active` na fase que cruza o meio da tela e atualiza número, contador e barra do painel fixo (`position: sticky`), sem estado React; a entrada (`from` + `freezeTransitions`) anima os **filhos** de cada linha, nunca a linha — a opacidade da linha é só do CSS `.step:not(.is-active)`, e um tween na linha deixaria `opacity: 1` inline por cima | `Process.tsx` |
...
**Opacidade por classe × tween.** Se um elemento tem estados de opacidade controlados por classe (fase ativa/inativa do Processo), nunca anime `opacity` nele com GSAP: o valor final fica inline e vence o CSS. Anime os filhos.

**Sticky + triggers.** O painel do `Process` é `position: sticky`; os triggers das fases usam a viewport (55%) como referência, então não dependem da altura do painel. Se adicionar conteúdo acima que carrega tarde (fontes, imagens grandes), chame `ScrollTrigger.refresh()` depois.
