...
| Intro | dobra grafite; palavras `opacity 0.18 → 1` com `scrub: 0.4`; um segundo `ScrollTrigger` (`toggleClass: is-lit`, no ponto em que o scrub termina) acende as palavras-chave em lime **só depois da frase inteira revelada** (transição CSS de 700 ms) | `Intro.tsx` |
