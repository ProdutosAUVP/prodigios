...
| Process | lista vertical; um `ScrollTrigger` por fase (`start: top 55%`, `end: bottom 55%`) liga `is-active` na fase que cruza o meio da tela e atualiza número, contador e barra do painel fixo (`position: sticky`), sem estado React; cada linha entra com `from` + `freezeTransitions` | `Process.tsx` |
