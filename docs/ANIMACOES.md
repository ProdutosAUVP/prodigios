...
| Hero | estado inicial escondido via `gsap.set` e entrada com `.to` quando `ready` vira `true`: linhas do título sobem da máscara (`.line-mask`, `expo.out`); o arco se revela de baixo para cima (`clipPath: inset(100% 0 0 0)` → `inset(0)`, `expo.inOut`) enquanto a foto "assenta" de `scale 1.14` para 1 em 2.4 s (Ken Burns); a foto menor sobe e o selo entra com `back.out`. Scroll: parallax por `data-depth` (arco 0.5, foto menor 1.1) e o texto recua | `Hero.tsx` |
