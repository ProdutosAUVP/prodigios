...
| **Lime** | `--lime` | **só** nos botões de CTA (navegação, Hero, envio do formulário), no check de sucesso do formulário, no ponto final das palavras do loader e em destaques pontuais (as palavras-chave da declaração da Intro — que só acendem depois da frase inteira revelada —, os números 1-2-3 da lista "O que você vai aprender" o ícone do card de trilha no hover o carimbo "Opcional" no instante em que surge, antes de repousar no preto, e os números da Cultura no hover) |
